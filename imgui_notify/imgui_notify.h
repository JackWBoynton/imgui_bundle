...
#include <instronimbus_client/font/FontAwesomeIcons.hpp>
...
#define NOTIFY_DEFAULT_DISMISS \
  3000// Auto dismiss after X ms (default, applied only of no data provided
      // in constructors)
...
#define NOTIFY_USE_DISMISS_BUTTON \
  true// If true, a dismiss button will be rendered in the top right corner
      // of the toast
...
private:
  ...
  std::function<void()> onButtonPress = nullptr;// A lambda variable, which will be executed when button in
...
public:
   ...
   *
   * @param onButtonPress std::fuction or lambda expression, which contains
...
public:
  ...
  /**
   * @brief Get the current phase of the toast notification based on the
   ...
   *         - ImGuiToastPhase::Expired: The notification has expired and
   * should be removed.
   */
  ...
  inline float getFadePercent()
  {
    ...
    if (phase == ImGuiToastPhase::FadeIn) {
      ...
      return (1.f
               - (((float)elapsed - (float)NOTIFY_FADE_IN_OUT_TIME - (float)this->dismissTime)
...
public:
  ...
  /**
   * @brief Creates a new ImGuiToast object with the specified type and
   * dismiss time.
   *
   * @param type The type of the toast.
   * @param dismissTime The time in milliseconds after which the toast should
   * be dismissed. Default is NOTIFY_DEFAULT_DISMISS.
   */
  ...
  /**
   * @brief Constructor for creating an ImGuiToast object with a specified
   ...
   * @param onButtonPress The lambda function to be executed when the button
   * is pressed.
   * @param format The format string for the content of the toast message.
   * @param ... The variable arguments to be formatted according to the format
   * string.
...
inline void RenderNotifications()
{
  ...
  for (size_t i = 0; i < notifications.size(); ++i) {
    ...
    char windowName[50];
#ifdef _WIN32
    snprintf(windowName, 10, "##TOAST%d", (int)i);
...
//         throw "Unsupported platform";
    ...
    // window, considering the main window size and location in relation to
    // the display
    ...
    {
      PushTextWrapPos(mainWindowSize.x / 3.f);// We want to support multi-line text, this will
      ...
      if (!NOTIFY_NULL_OR_EMPTY(title)) {
        ...
        Text("%s", defaultTitle);// Render default title text
      ...
      if (NOTIFY_USE_DISMISS_BUTTON) {
        // If a title or content is set, we want to render the button on
        // the same line
        ...
        if (CalcTextSize(content).x > GetWindowContentRegionMax().x) { scale = 0.95f; }
      ...
      if (currentToast->getOnButtonPress() != nullptr) {
        // If the button is pressed, we want to execute the lambda
        // function
